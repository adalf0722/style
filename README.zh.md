...
## 特色
- 11 種強烈風格主題（星戰、動森、電馭、吉卜力、極簡、樂高、復古電玩、Apple Vision、NASA 控制、奇幻 RPG、咖啡館）。
- 全域 ThemeSwitcher：以 token 控制色彩、字體、圓角、陰影、紋理、動畫。
- 模板庫：依主題提供可複製程式碼與即時預覽，含中英文風格說明（導覽/側欄、Hero/Card/Form/Table、圖表、徽章、空狀態、步驟、CTA、Stats）。
- 主題化頁面：首頁行銷版面、Admin、Dashboard（假數據長條 + 折線/tooltip）、Blog 清單/詳情（頭像、TOC、導覽）、一致的導覽列。
- `sample_style.md` 內的風格要點已映射到模板頁的「風格說明」區塊。
...
- Blog 資料：`src/pages/blog/blogData.ts`（使用佔位頭像 SVG）。
